...
from parser_ERSP import *
from tylib.exp.exp_ops import *
from tylib.exp.experiment_ERSP import Experiment
from tf_models.model_ERSP import Model
...
class CFExperiment(Experiment):
    ...
    def __init__(self, inject_params=None):
        ...
        self.show_metrics = ['MSE', 'RMSE', 'MAE', 'MSE_int', 'RMSE_int', 'MAE_int',
                             'Gen_loss', 'All_loss', 'Gen_loss', 'F1', 'ACC', 'Review_acc', 'AUC']
    ...
    def model_wrapper(self):
        ...
        if(self.args.rnn_type == 'DeepCoNN'):
            ...
            self.args.base_encoder = 'Flat'
        elif(self.args.rnn_type == 'ERSP'):
            self.args.rnn_type = 'RAW_MSE_ERSP_FN_FM'
            self.args.base_encoder = 'NBOW'
        elif(self.args.rnn_type == 'ERSP_mlp'):
            self.args.rnn_type = 'RAW_MSE_ERSP_FN_MLP'
            self.args.base_encoder = 'NBOW'
    ...
    def load_dataset(self, data_dir, dataset_type):
        ...
        for i in range(len(unique_item_id)):
            ...
            linedata = linedata[::-1]
            # 记录每个商店真实解释的编码
            item_review_1_encode.write(
                'item '+str(unique_item_id[i])+' review_1:'+str(linedata)+'\n')
    ...
    def _load_sets(self):
        ...
        if(self.no_text_mode == False):
            ...
            # load_reviews
            self.ui_review_dict, self.iu_review_dict = self.load_review_data(
                data_link, "review")
            self.ui_concept_dict, self.iu_concept_dict = self.load_review_data(
                data_link, "concepts")
            ...
            for j in range(self.args.batch_size):
                if (self.args.batch_size * i + j) < data_len:
                    ...
                    f = open(self.args.gen_dir + '/gen_review.' +
                             str(self.args.batch_size * i + j)+'.txt', 'w+')
